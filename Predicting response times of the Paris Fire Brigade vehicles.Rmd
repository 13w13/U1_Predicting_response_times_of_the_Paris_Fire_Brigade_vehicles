...
```{r}
data[data$rescue.center=="266323"]
```
...
#### Clean outliers for rescue.center
...
Then, we use the drop level function to drop levels where there is no sample.
```{r}
...
Apply same for x_test
```{r}
x_test$rescue.center <- subset(x_test$rescue.center, x_test$rescue.center != "266281")

x_test <- x_test[x_test$rescue.center != 266281,]

x_test <- x_test[x_test$rescue.center != 266294,]

x_test <- x_test[x_test$rescue.center != 266296,]

x_test <- x_test[x_test$rescue.center != 266321,]

x_test <- x_test[x_test$rescue.center != 266323,]

x_test <- x_test[x_test$rescue.center != 266324,]

table(droplevels(x_test$rescue.center))
...
- longitude.before.departure and longititude.intervention are highly correlated. We can remove longitude.before.departure
...
```{r}
res.pca=PCA(data.fe, scale.unit=TRUE, ncp=11, graph=F,quali.sup =c(22,21,20,13,11,10,9,6,5,4,3,1))
```
...
We can stop at Dimension 8 because we have reached a cumulative variance of 99%

Describe each axis according to the variables correlated with it using the dimdesc( ) 
...
barplot(inertia,ylab="% Unertia",names.arg=round(inertia,8))+
title("eigenvalues (inertias or variances of each component) in %")
```

```{r}
res.pca=PCA(data.variables.PCA, scale.unit=TRUE, ncp=8, graph=F)
...
trainset0<-trainset[,-c("delta.departure.presentation","delta.selection.presentation")]
testset0_y0<-testset$delta.selection.departure
testset0<-testset[,-c("delta.departure.presentation","delta.selection.presentation")]
```


Create one-hot matrix for cat variable. with sparse-matrix
```{r}
sparse_matrix <- sparse.model.matrix(delta.selection.departure ~ ., data = trainset0)[,-1]
```

Store into xgb Matrix
```{r}
dtrain <- xgb.DMatrix(sparse_matrix,label = trainset0$delta.selection.departure)
```
...
Predict
```{r}
sparse_matrix2 <- sparse.model.matrix(delta.selection.departure ~ ., data =testset0 )[,-1]
```
...
```{r}
dtest <- xgb.DMatrix(sparse_matrix2,label=testset0$delta.selection.departure)
```
...
testset0$delta.selection.departure %>% head
```
Compute MSE
```{r}
postResample(pred = pred_xgboost_y0, obs = testset0$delta.selection.departure)
...
trainset1<-trainset[,-c("delta.selection.departure","delta.selection.presentation")]
testset1_y1<-testset$delta.departure.presentation
testset1<-testset[,-c("delta.selection.departure","delta.selection.presentation")]
```

Create one-hot matrix for cat variable. with sparse-matrix
```{r}
sparse_matrix <- sparse.model.matrix(delta.departure.presentation ~ ., data = trainset1)[,-1]
```

Store into xgb Matrix
```{r}
dtrain <- xgb.DMatrix(sparse_matrix,label = trainset1$delta.departure.presentation)
```
Set parameters for our xgb tree
```{r}
...
Predict
```{r}
sparse_matrix2 <- sparse.model.matrix(delta.departure.presentation ~ ., data =testset1 )[,-1]
```
...
```{r}
dtest <- xgb.DMatrix(sparse_matrix2,label=testset1$delta.departure.presentation)
```
...
testset1$delta.departure.presentation %>% head
```
```{r}
postResample(pred = pred_xgboost_y1, obs = testset1$delta.departure.presentation)
```
...
### 6.5 Use feature imporatnce to create group for factor variables for futur reg
...
Predict
```{r}
sparse_matrix3 <- sparse.model.matrix( ~ ., data =x_test )[,-1]
```
...
```{r}
dtest <- xgb.DMatrix(sparse_matrix3)
```


```{r}
pred_xgboost_y0_final <- predict(gb_0_dt,dtest)
```

```{r}
pred_xgboost_y0_final %>% head
```

Y1
Predict
```{r}
pred_xgboost_y1_final <- predict(gb_1_dt,dtest)
```

```{r}
pred_xgboost_y1_final %>% head

```

Ys
```{r}
pred_xgboost_final_ys=pred_xgboost_y0_final+pred_xgboost_y1_final
pred_final_xgboost=data.frame(pred_xgboost_y0_final,pred_xgboost_y1_final,pred_xgboost_final_ys)

dim(x_test)

write.csv(pred_final_xgboost,"C:/Users/swp/Documents/_Perso/Cours/M2/U1. Analytical Theory, Methods and Models/Econometrics and Statitical Models/0.Project/1stsubmit.csv", row.names = FALSE)
```
...
Data_test <- dataset[-train_indices, ]
...
library(caret)
set.seed(4321)
trainIndex <- createDataPartition(dataset$delta.selection.departure, p = 0.8, list = FALSE, times = 1)
...
We split data. We will keep x_test for final prediction
```{r}
...
```{r}
trainingset<-trainingset[,-c("emergency.vehicle.selection")]
...
Data_train<-Data_train[,-c("delta.departure.presentation","delta.selection.presentation")]
...
lm <- lm(delta.selection.departure ~.,data = Data_train)
...
Test
```{r}
r_sq_1 <- summary(lm)$r.squared

prediction_1 <- predict(lm, newdata =Data_test[,-c("delta.departure.presentation","delta.selection.presentation")])

residuals_1 <- Data_test$delta.selection.departure - prediction_1
rmse_1 <- sqrt(mean(residuals_1^2))
print(paste0("R-squared for first model:", round(r_sq_1, 4)))
```


We can remove the latitude variable because he has the higher pvalue
```{r}
#Linear Regression,
...
lm1 <- lm(delta.selection.departure ~ 
           n 
...
We can remove the floor variable because he has the higher pvalue
```{r}
lm2 <- lm(delta.selection.departure ~ 
           n 
         + alert.reason.category
         + intervention.on.public.roads
         + location.of.the.event
...
summary(lm2)
```
We can remove the longitude intervention variable because he has the higher pvalue
```{r}
lm3 <- lm(delta.selection.departure ~ 
           n 
         + alert.reason.category
         + intervention.on.public.roads
         + location.of.the.event
...
We can remove the emergency vehicle type because """its has the higher pvalue
```{r}
lm4 <- lm(delta.selection.departure ~ 
           n 
         + alert.reason.category
         + intervention.on.public.roads
         + location.of.the.event
...
summary(lm4)
```
We can remove the rescue center variable because he has the higher pvalue
```{r}
lm5 <- lm(delta.selection.departure ~ 
           n 
         + alert.reason.category
         + intervention.on.public.roads
         + location.of.the.event
         ...
         + weekdays
         + hours
           ,data = trainingset, na.action = na.pass)

summary(lm5)
```
#### 6.2.2 Y0 : Multicolinéarity 
...
lm6 <- lm(delta.selection.presentation ~ 
           alert.reason.category
         + intervention.on.public.roads
         + floor
         + location.of.the.event
...
jarque.bera.test(lm4$residuals)
...
```
```{r}
qqnorm(log(lm5$residuals))
qqline(log(lm5$residuals))
```
...
vcov2 <- vcovHC(lm5, type = "HC1")

coeftest(lm5, vcov. = vcov2)
...
lm5$xlevels$alert.reason.category <- union(lm5$xlevels$alert.reason.category, levels(testset$alert.reason.category))
lm5$xlevels$intervention.on.public.roads <- union(lm5$xlevels$intervention.on.public.roads, levels(testset$intervention.on.public.roads))
lm5$xlevels$location.of.the.event <- union(lm5$xlevels$location.of.the.event, levels(testset$location.of.the.event))
lm5$xlevels$emergency.vehicle <- union(lm5$xlevels$emergency.vehicle, levels(testset$emergency.vehicle))
lm5$xlevels$departed.from.its.rescue.center <- union(lm5$xlevels$departed.from.its.rescue.center, levels(testset$departed.from.its.rescue.center))
lm5$xlevels$month <- union(lm5$xlevels$month, levels(testset$month))
lm5$xlevels$weekdays <- union(lm5$xlevels$weekdays, levels(testset$weekdays))
lm5$xlevels$hours <- union(lm5$xlevels$hours, levels(testset$hours))
...
```{r}
pred_reg_y0 <- lm5 %>% predict(newdata = testset)

R2 = R2_Score(pred_reg_y0, testset$delta.selection.departure)
R2

head(ConfusionMatrix(pred_reg_y0, testset$delta.selection.departure))



# Model performance
data.frame(
  RMSE = RMSE(pred_reg_y0, testset$delta.selection.departure),
  R2 = R2_Score(pred_reg_y0, testset$delta.selection.departure)
)
```
...
```{r}
trainingset<-trainingset[,-c("emergency.vehicle.selection","emergency.vehicle")]
...
trainingset<-trainingset[,-c("delta.departure.presentation","delta.selection.presentation")]
```
...
lm <- lm(delta.selection.departure ~.,data = trainingset)
...
##### 6.3.3.2 Are residuals normally distributed?
...
##### 6.3.3.3 Are residuals homoskedastic?
...
##### 6.3.3.4 Are the residuals correlated ?
...
```{r}
pred_reg_y0 <- lm5 %>% predict(testset)
# Model performance
data.frame(
  RMSE = RMSE(pred_reg_y0, testset$delta.selection.departure),
  R2 = R2(pred_reg_y0, testset$delta.selection.departure)
)
```
