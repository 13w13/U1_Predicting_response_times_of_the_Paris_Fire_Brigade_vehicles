...
## 1.5 Convert qualitative variables into factor
```{r}
...
Then, let's store quanti and variable variable
```{r}
data.quali<-data[,c(3,4,5,7,11,12,16,18)]
data.quanti.y<-data[,-c(3,4,5,7,11,12,16,18)] #with Ys
data.quanti<-data[,-c(3,4,5,7,11,12,16,18,25,26,27)]
```

## 1.6 Convert GPS data
First GPS.track.departure.presentation is stored as set of character lat and log
```{r}
...
Get metric in indicators Names : Using stringr library and str_extract_all in order to get in a matrix all what is inside parenthesis

# Get the parenthesis and what is inside
metrics <- as.data.frame(str_extract_all(GPS.track.departure.presentation.lat, "",.*$")", simplify = TRUE))
...
We have 2 dependant variables and 1 gloabl variable
...
Delta Departure to presentation
...
data quanti + ys
```{r}
data.quali.y=data[,c(3,4,5,7,11,12,16,18,25,26,27)]
```


```{r}
pairs(delta.selection.departure~.,data=data.quanti,
...
```{r}
colnames(data.clean)
data.variables <- data.clean[,c(3,5,6,7,8,9,11,12,16,17,18,19,20,23,24,25,27,28)]
colnames(data.variables)
data.variables$alert.reason.category <- as.factor(data.variables$alert.reason.category )
data.variables$intervention.on.public.roads <- as.factor(data.variables$intervention.on.public.roads)
data.variables$emergency.vehicle.type <- as.factor(data.variables$emergency.vehicle.type)
data.variables$rescue.center <- as.factor(data.variables$rescue.center )
data.variables$departed.from.its.rescue.center <- as.factor(data.variables$departed.from.its.rescue.center)


colnames(data.quali)
data.quanti <- data.variables[,c(3,5,6,10,12,13,14,15,16,17,18)]
