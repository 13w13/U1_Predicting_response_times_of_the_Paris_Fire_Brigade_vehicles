...
```{r}
colnames(data.clean)
data.variables <- data.clean[,c(3,5,6,7,8,9,11,12,16,17,18,19,20,23,24,28)]
colnames(data.variables)
colnames(data.quanti)
data.quanti <- data.variables[,c(3,5,6,10,12,13,14,15,16)]
data.quali <- data.variables[,-c(3,5,6,10,12,13,14,15,16)]
```
#coorelation
```{r}
cor(data.quanti)
...
lm <- lm(data.variables.without.cor$delta.selection.presentation~.,data = data.variables.without.cor)
summary(lm)
```
