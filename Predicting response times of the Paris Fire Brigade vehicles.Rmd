...
### 6.5 Use feature imporatnce to create group for factor variables for futur regression
...
We regroup th types of emergency vehicle, the rescue centers and thealert reason category
```{r}
...
trainset0.regroup = trainset0.regroup%>%
    mutate(alert.reason.category.regroup = case_when(alert.reason.category == 2 ~  1,
                                  alert.reason.category == 3 ~  1,
                                  TRUE ~ 0))
...
Y0
Predict
Prb : not the same one hot encoding ...114 column missing
...
```{r}
object <- xgb.Booster.complete(gb_0_dt, saveraw = FALSE)
