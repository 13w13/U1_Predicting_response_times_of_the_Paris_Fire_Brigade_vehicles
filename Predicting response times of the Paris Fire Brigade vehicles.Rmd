...
```{r}
dtrain <- xgb.DMatrix(sparse_matrix,label = trainset$delta.selection.departure)
```
...
Store into xgb Matrix
```{r}
dtrain <- xgb.DMatrix(sparse_matrix,label = trainset$delta.departure.presentation)
```
Set parameters for our xgb tree
```{r}
...
```{r}
namevector <- c("delta.selection.departure")
x_test[ , namevector] <- N.A


x_test[,"delta.selection.departure"] <- 0
x_test$delta.selection.departure<- "1"
```

```{r}
gdxg<-x_test[,-c("delta.selection.departure")]
```
...
Predict
```{r}
sparse_matrix3 <- sparse.model.matrix( ~ ., data =gdxg)[,-1]
```
...
```{r}
dtest <- xgb.DMatrix(sparse_matrix3,label=x_test$delta.selection.departure)
```
