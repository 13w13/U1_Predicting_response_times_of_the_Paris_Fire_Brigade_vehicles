...
sparse_matrix <- sparse.model.matrix(delta.selection.departure ~ ., data = Data_train)[,-1]
X_train_dmat = xgb.DMatrix(sparse_matrix, label = Data_train$delta.selection.departure)
```

```{r}
dtrain <- xgb.DMatrix(sparse_matrix,label = Data_train$delta.selection.departure)
```
...
```{r}
sparse_matrix <- sparse.model.matrix(delta.selection.departure ~ ., data =Data_test )[,-1]
```
...
```{r}
mse = mean((Data_test$delta.departure.presentation - test_preds)^2)
```

```{r}
model <- xgb.dump(gb_dt)
```
...
```{r}
library(tibble)
imp_matrix <- as.tibble(xgb.importance(feature_names = colnames(Data_test %>% select(-delta.selection.departure)), model = gb_dt))
...
```{r}
Data_test<-Data_test[,-c("emergency.vehicle.selection")]
...
xgbc = xgboost(data = as.matrix(Data_train[,-c("delta.selection.departure")]), label=(Data_train$delta.selection.departure),max.depth = 2, nrounds = 10)
...
```{r}
pred_y = predict(xgbc, Data_test[,-c("delta.departure.presentation","delta.selection.presentation")])

mse = mean((Data_test$delta.departure.presentation - pred_y)^2)
```
