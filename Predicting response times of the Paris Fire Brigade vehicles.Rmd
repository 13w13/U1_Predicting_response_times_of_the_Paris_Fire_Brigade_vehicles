...
```{r}
data.fe$month <-month(as.Date(as.character(data.fe$date.key.s�.lection), "%Y%m%d"))

data.fe$weekdays <-weekdays(as.Date(as.character(data.fe$date.key.s�.lection), "%Y%m%d"))
```
...
```{r}
data.fe$hours <-as.hms(formatC(as.integer(data.fe$time.key.s�.lection), big.mark = ":", big.interval = 2L))
...
No time so we decide to remove gps datas from our analysis. 
